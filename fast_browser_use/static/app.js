...
function render() {
  if (!state) return;
  $("helper").textContent = `Local Qwen3.5-9B · ${state.text_model}`;
  ...
  const labels = {
    ...
    done: "Qwen3.5-9B reports complete · inspect the page",
...
$("task-form").addEventListener("submit", (event) => {
  ...
  perform(
    () =>
      call("reset", { scenario: $("scenario").value, goal: $("goal").value, url: $("custom-url").value }),
    "Opening a fresh browser…",
  );
});
$("scenario").addEventListener("change", () => {
  $("custom-url").hidden = $("scenario").value !== "custom";
  $("goal").value = goals[$("scenario").value] || "";
});
$("choose").addEventListener("click", () =>
  perform(() => call("predict"), "Qwen3.5-9B is comparing the actions…"),
);
...
$("download").addEventListener("click", () => {
  ...
  a.href = url;
  a.download = "fast-browser-use-trace.json";
  a.click();
